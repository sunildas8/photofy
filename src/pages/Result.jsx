...
const Result = () => {
  ...
  return (
    <motion.form onChange={onSubmitHandler} className='flex flex-col justify-center items-center min-h-[90vh]'
    ...
    > 
      <div>
        <div className='relative'>
          ...
          <span className={`absolute bottom-0 left-0 ${loading ? 'bg-bar h-1 w-full transition-all duration-[10s]' : 'w-0'}`}/>
        </div>
      ...
      {isImageLoaded && 
        <div className='flex flex-wrap justify-center gap-2 text-white text-sm p-0.5 mt-10 rounded-full'>
          <p onClick={(()=>{setIsImageLoaded(false)})} className='bg-transparent text-black border border-zinc-900 px-8 py-3 rounded-full cursor-pointer'>Generate Another</p>
